...
const template = (onSubmit, errorMessage = '', errorType = {}, onReveal, revealPass) => html`
            ...
            <div class="col-md-4">
                ...
                <div class="form-group">
                    ...
                    <input class=${'form-control' + (errorType.password ? ' is-invalid' : ' is-valid' )} id="password"
                        type=${revealPass ? 'text' : 'password' } name="password">
                    <label><input @change=${onReveal} type="checkbox">Reveal</label>
                </div>
                <div class="form-group">
...
export function registerPage(ctx) {
    const update = (errorMessage, errorType, revealPass) => ctx.render(template(onSubmit, errorMessage, errorType, onReveal, revealPass));

    update();

    function onReveal(e) {
        e.preventDefault();
        update(null, {}, e.target.checked);
    }
